...
A v2 stroke has exactly `tool`, `style`, `width`, `opacity`, and `points`. `tool` is `pencil`, `pen`, `marker`, or `highlighter`. `style`, `width`, and `opacity` must be one of that tool’s fixed choices shown by the editor. Every point is `[x, y, pressure]`; all three values are finite numbers from 0 through 1 relative to the section box. Export rounds point values to four decimal places and sorts section/layout records for reviewable diffs.
